...
export default function ServicesPage() {
  ...
  const shown = filter === 'All' ? publicModules : publicModules.filter((m) => m.category === filter)
  const ordered = [...shown].sort((a, b) => a.no - b.no)

  useEffect(() => { document.title = 'Services · HomeSIEM' }, [])
      ...
      <main className="pt-[72px]">
        ...
        <section className="bg-slate-900 py-20 lg:py-28">
          <div className="container-x">
            <span className="eyebrow text-brand-300">Services</span>
            <h1 className="mt-4 text-4xl font-extrabold tracking-tight text-white sm:text-5xl">
              Everything inside the dashboard
            </h1>
            <p className="mt-5 max-w-2xl text-lg leading-relaxed text-slate-200/90">
              Twenty-five modules covering collection, detection, AI analysis, response and posture.
              Every card below is a live section of the platform.
            </p>
          </div>
        </section>
